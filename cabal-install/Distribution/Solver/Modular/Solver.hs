...
#ifdef DEBUG_TRACETREE
instance GSimpleTree (Tree a QGoalReason) where
  fromGeneric = go
    where
      go :: Tree a QGoalReason -> SimpleTree
      ...
      -- Show package choice
      goP :: QPN -> POption -> Tree a QGoalReason -> (String, SimpleTree)
      ...
      goFS :: Bool -> Tree a QGoalReason -> (String, SimpleTree)
      goFS val subtree = (show val, go subtree)

      -- Show goal choice
      goG :: Goal QPN -> Tree a QGoalReason -> (String, SimpleTree)
