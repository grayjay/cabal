...
convIPId pn' comp idx ipid =
  case SI.lookupUnitId idx ipid of
    ...
    Just ipi -> let (pn, i) = convId ipi
                in  Just (D.SimpleDep (PackageDep False pn (Fixed i (P pn'))) comp)
...
convGPD os arch cinfo strfl solveExes pi
  ...
  let
    ...
    flagged_deps
       ...
       ++ prefix (\deps -> Flagged (FlagDeps (StanzaInfo (SN pi TestStanzas))  deps []))
            (L.map  (\(nm, ds) -> conv (ComponentTest nm)  testBuildInfo      ds) tests)
       ++ prefix (\deps -> Flagged (FlagDeps (StanzaInfo (SN pi BenchStanzas)) deps []))
...
flagInfo :: StrongFlags -> [PD.Flag] -> FInfoMap
...
convCondTree :: PackageDescription -> OS -> Arch -> CompilerInfo -> PI PN -> FInfoMap ->
                Component ->
                (a -> BuildInfo) ->
                IPNs ->
                SolveExecutables ->
...
convCondTree pkg os arch cinfo pi@(PI pn _) fds comp getInfo ipns solveExes@(SolveExecutables solveExes') (CondNode info ds branches) =
                 concatMap
                    (\d -> filterIPNs ipns d (D.SimpleDep (convLibDep pn d) comp))
                    ds  -- unconditional package dependencies
              ++ L.map (\e -> D.SimpleConstraint (Ext  e) comp) (PD.allExtensions bi) -- unconditional extension dependencies
              ++ L.map (\l -> D.SimpleConstraint (Lang l) comp) (PD.allLanguages  bi) -- unconditional language dependencies
              ++ L.map (\(PkgconfigDependency pkn vr) -> D.SimpleConstraint (Pkg pkn vr) comp) (PD.pkgconfigDepends bi) -- unconditional pkg-config dependencies
              ...
              ++ [ D.SimpleDep (convExeDep pn exeDep) comp
                 | solveExes'
...
--
--      3. The flag defaults 'FInfoMap' so that we can populate
...
convBranch pkg os arch cinfo pi@(PI pn _) fds comp getInfo ipns solveExes (CondBranch c' t' mf') =
  ...
  where
    ...
    go (Var (Flag fn)) t f = extractCommon t f ++ [Flagged $ FlagDeps (PkgFlagInfo (FN pi fn) (fds ! fn)) t f]
    go (Var (OS os')) t f
      | os == os'      = t
      | otherwise      = f
    ...
    extractCommon ps ps' = [ D.SimpleDep (PackageDep is_exe1 pn1 (Constrained [(vr1 .||. vr2, P pn)])) comp
                           | D.SimpleDep (PackageDep is_exe1 pn1 (Constrained [(vr1, _)])) _ <- ps
                           , D.SimpleDep (PackageDep is_exe2 pn2 (Constrained [(vr2, _)])) _ <- ps'
                           , pn1 == pn2
                           , is_exe1 == is_exe2
                           ]

-- | Convert a Cabal dependency on a library to a solver-specific dependency.
convLibDep :: PN -> Dependency -> PackageDep PN
convLibDep pn' (Dependency pn vr) = PackageDep False {- not exe -} pn (Constrained [(vr, P pn')])
...
convExeDep :: PN -> ExeDependency -> PackageDep PN
convExeDep pn' (ExeDependency pn _ vr) = PackageDep True pn (Constrained [(vr, P pn')])

-- | Convert setup dependencies
convSetupBuildInfo :: PI PN -> SetupBuildInfo -> FlaggedDeps PN
convSetupBuildInfo (PI pn _i) nfo =
    L.map (\d -> D.SimpleDep (convLibDep pn d) ComponentSetup) (PD.setupDepends nfo)
