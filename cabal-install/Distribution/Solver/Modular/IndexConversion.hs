...
convBranch flags dr pkg os arch cinfo pn fds comp getInfo ipns solveExes (CondBranch c' t' mf') =
  ...
  where
    ...
    extractCommon ps ps' =
        ...
        -- conditional.
        [ D.Simple (LDep (unionDRs vs1 vs2) (Dep comp1 (Constrained $ vr1 .||. vr2))) comp
        | D.Simple (LDep vs1                (Dep comp1 (Constrained vr1))) _ <- ps
        , D.Simple (LDep vs2                (Dep comp2 (Constrained vr2))) _ <- ps'
        , comp1 == comp2
        ]
