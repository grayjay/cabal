...
data ValidateState = VS {
  ...
  -- package.
  requiredComponents  :: Map QPN CompDeps,
...
data PkgDep = PkgDep (DependencyReason QPN) ExposedComponent QPN CI

-- | Map from component name to one of the reasons that the component is
-- required.
type CompDeps = Map ExposedComponent (DependencyReason QPN)
  ...
  where
    ...
    goP qpn@(Q _pp pn) (POption i _) r = do
      ...
      case mfr of
        ...
        Nothing ->
          let newDeps :: Either Conflict (PPreAssignment, Map QPN CompDeps)
              newDeps = do
                nppa <- mnppa
                rComps' <- extendRequiredComps aComps rComps newactives
    ...
    goF qfn@(FN qpn _f) b r = do
      ...
      let newactives = extractNewDeps (F qfn) b npfa psa qdeps
          mNewRequiredComps = extendRequiredComps aComps rComps newactives
...
checkComponentsInNewPackage :: Map QPN CompDeps
                            -> QPN
                            -> [ExposedComponent]
                            -> Either Conflict ()
...
extend extSupported langSupported pkgPresent newactives ppa = foldM extendSingle ppa newactives
  where
    ...
    extendSingle a (LDep dr (Dep (PkgComponent qpn comp) ci)) =
      let mergedDep = M.findWithDefault (MergedDepConstrained []) qpn a
      in  case (\ x -> M.insert qpn x a) <$> merge mergedDep (PkgDep dr comp qpn ci) of
...
extendWithPackageChoice (PI qpn i) ppa =
  let mergedDep = M.findWithDefault (MergedDepConstrained []) qpn ppa
      newChoice = PkgDep (DependencyReason qpn M.empty S.empty) ExposedLib qpn (Fixed i)
...
merge (MergedDepFixed comp1 vs1 i1) (PkgDep vs2 comp2 p ci@(Fixed i2))
  | i1 == i2  = Right $ MergedDepFixed comp1 vs1 i1
  | otherwise =
      Left ( (CS.union `on` dependencyReasonToCS) vs1 vs2
           , ( ConflictingDep vs1 comp1 p (Fixed i1)
             , ConflictingDep vs2 comp2 p ci ) )

merge (MergedDepFixed comp1 vs1 i@(I v _)) (PkgDep vs2 comp2 p ci@(Constrained vr))
  | checkVR vr v = Right $ MergedDepFixed comp1 vs1 i
  | otherwise    =
      Left ( (CS.union `on` dependencyReasonToCS) vs1 vs2
           , ( ConflictingDep vs1 comp1 p (Fixed i)
             , ConflictingDep vs2 comp2 p ci ) )

merge (MergedDepConstrained vrOrigins) (PkgDep vs2 comp2 p ci@(Fixed i@(I v _))) =
  ...
  where
    ...
    go ((vr, comp1, vs1) : vros)
       ...
       | otherwise    =
           Left ( (CS.union `on` dependencyReasonToCS) vs1 vs2
                , ( ConflictingDep vs1 comp1 p (Constrained vr)
                  , ConflictingDep vs2 comp2 p ci ) )

merge (MergedDepConstrained vrOrigins) (PkgDep vs2 comp2 _ (Constrained vr)) =
...
-- requires an component that is missing from one of the previously chosen
-- packages.
extendRequiredComps :: Map QPN [ExposedComponent]
                    -> Map QPN CompDeps
                    -> [LDep QPN]
                    -> Either Conflict (Map QPN CompDeps)
extendRequiredComps available = foldM extendSingle
  where
    extendSingle :: Map QPN CompDeps -> LDep QPN -> Either Conflict (Map QPN CompDeps)
    extendSingle required (LDep dr (Dep (PkgComponent qpn comp) _)) =
      let compDeps = M.findWithDefault M.empty qpn required
      in -- Only check for the existence of the comp if its package has already
         -- been chosen.
