...
assign :: Tree a b -> Tree Assignment b
assign tree = cata go tree $ A M.empty M.empty M.empty
  where
    go :: TreeF a b (Assignment -> Tree Assignment b)
                 -> (Assignment -> Tree Assignment b)
...
backjumpAndExplore :: EnableBackjumping
                   -> CountConflicts
                   -> Tree a QGoalReason -> Log Message (Assignment, RevDepMap)
