...
-- have to add.
addLinking :: Tree a b -> Tree a b
addLinking = (`runReader` M.empty) .  cata go
  where
    go :: TreeF a b (Linker (Tree a b)) -> Linker (Tree a b)
    ...
    goP :: QPN -> POption -> Linker (Tree a b) -> Linker (Tree a b)
...
validateLinking :: Index -> Tree a b -> Tree a b
