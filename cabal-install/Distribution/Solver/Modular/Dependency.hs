...
-- | A specific component within a package. This type is used to represent
...
qualifyDeps QO{..} (Q pp@(PackagePath ns q) pn) = go
  where
    ...
    goD (Dep dep@(PkgComponent depPkg depComp) ci) comp
      | isExe depComp = Dep (Q (PackagePath ns (QualExe   pn depPkg)) <$> dep) ci
      | qBase depPkg  = Dep (Q (PackagePath ns (QualBase  pn       )) <$> dep) ci
      | qSetup comp   = Dep (Q (PackagePath ns (QualSetup pn       )) <$> dep) ci
      | otherwise     = Dep (Q (PackagePath ns inheritedQ           ) <$> dep) ci

    isExe (ExposedExe _) = True
    isExe ExposedLib     = False
