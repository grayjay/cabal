...
-- using 'llength'.
lchoices :: Tree a b -> Int
lchoices (PChoice    _ _     ts) = W.llength (W.filter active ts)
lchoices (FChoice    _ _ _ _ ts) = W.llength (W.filter active ts)
lchoices (SChoice    _ _ _   ts) = W.llength (W.filter active ts)
...
cataM :: Monad m => (TreeF a b c -> m c) -> Tree a b -> m c
...
anaM :: Monad m => (c -> m (TreeF a b c)) -> c -> m (Tree a b)
